myApp.controller('advertisementsCtrl', ['$scope','$rootScope','$uibModal','$cookies','services','factories','$state','$loading','$stateParams',function ($scope,$rootScope,$uibModal,$cookies,services,factories,$state,$loading,$stateParams) {
    ...
    $scope.listEventsFunction = function (limit,value,flagy) {
        ...
        services.getadvertisementsService($scope.listOptions,$scope.start, $scope.end,$scope.start1, $scope.end1,value,function (response, status) {
            if (status == 1) {
    ...
    $scope.deleteAdv = function (id) {
        ...
        factories.confirm(message,function () {
            services.deleteAdvService (id,function (response,status) {
                if(status == 1){
...
myApp.controller('advertisementsaddCtrl', ['$scope','$rootScope','$uibModal','$cookies','services','factories','$state','$loading','$stateParams',function ($scope,$rootScope,$uibModal,$cookies,services,factories,$state,$loading,$stateParams) {
    ...
    else if($stateParams.type=='edit')
    {
        ...
        $scope.listEventsFunction = function (limit,value) {
            ...
            $loading.start('EventsLoad');
            services.getadvertisementsService({skip:0,limit:1,_id:$stateParams.id},null, null,null, null,null,function (response, status) {
                if (status == 1) {
    ...
    $scope.addTemplateSubmit = function (isValid) {
        if (isValid) {
           ...
           $loading.start('AddTemplateLoad');
            ...
            services.createadvertisementService($scope.addTemplate, $scope.lat , $scope.lng, $scope.city, function (response, status) {
                if (status == 1) {
